...
function fixture(): DomainData {
  ...
  data.colorLabels.push({ id: "c1", name: "Fontos", color: "#e0567a", order: 1000 });
  return data;
}
...
describe("parseImport validation", () => {
  ...
  it("caps custom labels at 12", () => {
    const data = fixture();
    const exported = JSON.parse(exportJson(data, 1));
    for (let i = 0; i < 20; i += 1) {
      exported.data.colorLabels.push({ id: `x${i}`, name: null, color: "#fff", order: i });
    }
    const result = parseImport(JSON.stringify(exported));
    expect(result.ok).toBe(true);
    if (result.ok) expect(result.data.colorLabels).toHaveLength(12);
  });
